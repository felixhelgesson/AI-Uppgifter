...
public class DataContext {
	...
	public boolean CheckMoveToNearestPill(Game game)
	{
		int[] pills=game.getPillIndices();
		int[] powerPills=game.getPowerPillIndices();
	
		if(pills.length !=0||powerPills.length !=0)
		{
			return true;
		}				
		return false;
	...
	public boolean IsPillEaten(Game game)
	{
		return false;
	}
	
	public boolean IsPillClose(Game game)
	{
		...
		for(int i = 0; i<targetsArray.length; i++)
		{
		//game.getShortestPathDistance(current, targets.get(i))
			if(game.getShortestPathDistance(current, targets.get(i))<100)
			{
				return true;
			}
		}
